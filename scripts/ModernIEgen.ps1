...
$Logfile = "$global:Path\ModernIEgen_Instance_$Instance.log"
...
function Create-LockFile ($vms) {
    ...
    $vmslock | ConvertTo-Json -depth 999 | Out-File "$global:Path\ModernIEgen.json.lock"
...
function Compress($software, $browser, $windows, $oshost) {
    ...
    If ($software -eq "VirtualBox") {
        ...
        $sourceBox = "$global:Path\..\vms\output\Vagrant\ModernIE.box"
...
function Check-RestartEnableHyperV() {
    If ($HyperVEnabled -eq 0) { 
        ...
        Save-HyperVStatus 1
        Invoke-Expression "bcdedit /set hypervisorlaunchtype on"
...
function Save-HyperVStatus($isenabled){
    @{hypervenabled=$isenabled;} | ConvertTo-Json | Out-File "$global:Path\ModernIEgen.cfg"
}

function Create-RestartRegistryEntry {
    $RegistryKey = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce"
    $RegistryEntry = "ContinueModernIEgen"
...
function Load-ModernIEConfig {
    $strFileName="$global:Path\ModernIEgen.cfg"
...
function Load-ModernIEProjectConfig {
    $global:Config = (Get-Content "$global:Path\ModernIEgen.json" -Raw) | ConvertFrom-Json
...
function Start-BuildPackerTemplates {
    ...
    if ($Build -eq $False) {
        Get-Content "$global:Path\ModernIEgen.json.lock" -Raw
    }
}

function Start-GenerationProcess {

    LogWrite "ModernIEgen process started."

    LogWrite "Build ID: $global:buildId"

    $lock = (Get-Content "$global:Path\ModernIEgen.json.lock" -Raw) | ConvertFrom-Json
    ...
    ForEach-Object {
        ...
        $lock | ConvertTo-Json | Out-File "$global:Path\ModernIEgen.json.lock" 
    }
...
function Generate-SofwareListJson () {
    Invoke-Expression "..\bin\VMSGen\VMSGen $global:Path\ModernIEgen.json"
...
function Send-NotificationEmail () {
    ...
    $subject = "ModernIEgen Build $global:buildId $subjectstatus"
...
If ($Continue -eq $False -and (Test-Path $LogFile)) { Clear-Content $Logfile }
Load-ModernIEProjectConfig
Load-ModernIEConfig
If ($Download -eq $True) { Download-ISOs }
